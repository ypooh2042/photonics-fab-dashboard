...
- `params`: free-form key-value object, but keep it to what genuinely varies per mention, not the recipe's fixed condition:
  ...
  - **`resist_thickness_nm`, `pre_strip_step_height_nm`, `post_strip_step_height_nm` feed an automatic selectivity calculation downstream, so a wrong or fabricated value here silently plots a false data point.** For each of these three fields, independently: include it **only** if the note reports an actual measured number for it (alphastep/AFM or equivalent), using exactly that key name (numeric, `_nm` suffix — never a variant spelling). If the note doesn't mention it, says it wasn't measured (e.g. "측정 안함", "알파스텝 정상 측정 불가"), or only gives a vague range/description instead of a single clean number, **omit that key entirely** — do not write it in as a string (`"측정 안함"`, `"39~39.5nm"`) and do not guess a number to fill the gap. A missing key correctly means "not measured"; a string or invented value in a numeric field is worse than a missing one, since it can silently break the downstream calculation or (if it happens to parse as a number) produce a selectivity point that never should have existed.
