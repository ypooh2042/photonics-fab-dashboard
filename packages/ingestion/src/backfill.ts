import { listDailyNotes, markNoteProcessed } from "./scan-notes.js";
...
function parseArgs() {
  ...
  return { from: get("--from", "2026-06-01"), to: get("--to", "2026-07-31") };
}
...
async function main() {
  ...
  const notes = listDailyNotes()
    .map((n) => ({ ...n, date: inferNoteDate(n.relPath.split("/").pop()!) }))
    .filter((n) => n.date >= from && n.date <= to)
    .sort((a, b) => a.date.localeCompare(b.date));

  console.log(`[backfill] ${notes.length} notes in range ${from}..${to}`);

  let ok = 0;
  let failed = 0;
  for (const note of notes) {
    const filename = note.relPath.split("/").pop()!;
    process.stdout.write(`[backfill] ${note.date}  ${filename} ... `);
    try {
      ...
      upsertNoteExtraction(extraction, note.relPath, note.date);
      markNoteProcessed(note, "success", { model: "claude-sonnet-5 (via claude -p)" });
      console.log(
        `ok (${extraction.chip_runs.length} chip_runs, ${extraction.recipe_entries.length} recipe_entries)`,
      );
      ok++;
    } catch (err) {
