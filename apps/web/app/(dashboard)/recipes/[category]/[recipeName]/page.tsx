...
export default async function RecipeDetailPage({
  ...
  return (
    <div className="max-w-3xl">
      ...
      {description && (
        <div className="mb-4 rounded-lg border border-black/10 dark:border-white/15 p-4">
          <h3 className="font-medium text-sm mb-2">
            {logOnly ? t("recipeDescriptionLabel") : t("recipeDescriptionFixedLabel")}
          </h3>
      ...
      {entries.length === 0 ? (
        <p className="mt-6 text-sm opacity-60">{t("noUsageHistory")}</p>
      ) : logOnly ? (
        <div className="mt-6">
          <h3 className="font-medium text-sm mb-2">{t("usageDateLogHeading")}</h3>
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="text-left border-b border-black/10 dark:border-white/15">
              <th className="py-2 pr-4">{t("dateLabel")}</th>
              <th className="py-2 pr-4">{t("paramsLabel")}</th>
            </tr>
          </thead>
          <tbody>
            {entries
